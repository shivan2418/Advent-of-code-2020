...
def preparse_input(input_str):
    ...
    return parse_rules(rules), input

def parse_rules(rules):
    rules_dict = {}
    for line in rules:
        index, rule = line.split(":")
        rules_dict[index]=rule.strip()
    return rules_dict
...
def is_valid(input_str):
    ...
    for line in input:
        if re.match(rule_to_apply,line) is not None:
            correct_lines+=1
...
def parse_rules_recursively(rules_dict):
    ...
    while any([l.isdigit() for l in all_values]):
        ...
        for key, value in rules_dict.items():
            ...
            for rule in set(individual_rules):
                pattern = f'(?<!\d){rule}(?!\d)'
