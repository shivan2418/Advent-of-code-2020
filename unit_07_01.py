rules = """muted lavender bags contain 5 dull brown bags, 4 pale maroon bags, 2 drab orange bags.
...
def format_capacity(string):
    ...
    for line in lines:
        line = line.strip()
        n = re.match(r'[\d]{1}', line).group(0)
        bag_name = line.split(n)[-1].strip()
        n = int(n.strip())
        outputs.append( (n,bag_name) )
    return outputs

def get_bags_that_can_contain_bag(bag_to_contain:Bag)->list:
    can_hold_bag_bags = [b for b in all_bags.values() if bag_to_contain in b.can_hold_bags.keys()]
    return can_hold_bag_bags

def get_all_bags_from_source_bag(source_bag:Bag):
    '''Returns a list of all bags that can contain this bag directly or indirectly'''
    can_hold_source_bag = get_bags_that_can_contain_bag(source_bag)
    total_bags.extend(can_hold_source_bag)
    if len(can_hold_source_bag)>0:
        for bag in can_hold_source_bag:
            get_all_bags_from_source_bag(bag)
...
# recursive get all bags that can hold the source bag
get_all_bags_from_source_bag(shiny_gold_bag)

# print out out the length of SET of the total bags since there are duplicates
