...
def letter_in_all_lists(letter, group_lists):
...
def get_num_q_answered(group_lists:list)->int:
    '''Only questions that everyone answered yes to'''

    # only questions that are present in all list is something everyone can answer yes to

    group_lists.sort(key=lambda x:len(x))
    longest_line = group_lists[-1]
    answered=0

    for letter in longest_line:
        if letter_in_all_lists(letter,group_lists[:-1]):
            answered+=1

    return answered

total_num_question = 0
group_lines = []
for line in questions:
    if line=="":
        total_num_question+=get_num_q_answered(group_lines)
        group_lines=[]
    else:
