...
def parse_passwords():
    ...
    for line in passwords.split('\n'):
        ...
        contain = policy.split(' ')[0]
        must_pos,must_not_pos = contain.split('-')
        must_pos = int(must_pos)
        must_not_pos = int(must_not_pos)
        password = line.split(":")[-1].strip()

        ps_str = password[must_pos-1]+password[must_not_pos-1]
        if Counter(ps_str)[letter]==1:
            valid_passwords+=1
