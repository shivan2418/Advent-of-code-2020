...
def valid_passports():
    ...
    for line in lines:
        if line == "":
            ...
            tmp_passport.update(parse_line(line))
    else:
        passports.append(tmp_passport)
    full_passports = 0
    reject=0
...
def passport_is_valid(pas:dict):
    required_fields = ['byr','iyr','eyr','hgt','hcl','ecl','pid']
    key_validators={'byr':validate_byr,'iyr':validate_iyr,'eyr':validate_eyr,'hgt':validate_hgt,'hcl':validate_hcl,'ecl':validate_ecl,'pid':validate_pid,'cid':validate_cid}
    keys_ok = all([key in pas.keys() for key in required_fields])
    if keys_ok is False: return False

    for key,value in pas.items():
        validation_func = key_validators[key]
        if validation_func(value) is False:return False
...
def validate_eyr(input):
    return len(input)==4 and 2020<=int(input)<=2030

def validate_pid(input):
    return len(input)==9 and all([n.isdigit for n in input])

def validate_cid(input):
    '''Always good, does not matter'''
    return True

def validate_hgt(input):
    hgt_type = input[-2:]
    if hgt_type not in ['in','cm']: return False
...
def validate_hcl(input):
    pat = r'#[\da-f]{6}' # pound sign followed by 6 chars that are either a digit or a letter betweeen a and f
    return re.match(pattern=pat,string=input) is not None
